...
[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.20108648.svg)](https://doi.org/10.5281/zenodo.20108648)
...
```bash
docker build -t cellular-inference-mesh:0.3.2 .
docker run --rm cellular-inference-mesh:0.3.2    # smoke run by default
docker run --rm -v $(pwd)/output:/app/output \
    cellular-inference-mesh:0.3.2 \
...
The canonical SHA-256 of every `src/*.py` file is the **single source of truth** in [`docs/hash-chain.md`](./docs/hash-chain.md), which is auto-generated by `scripts/regen_hash_chain.py` from `output/experiment_provenance.json`. To verify locally:

```bash
shasum -a 256 src/__init__.py src/config.py src/simulation.py src/main.py src/report.py
```

Then compare against the table in [`docs/hash-chain.md`](./docs/hash-chain.md#source-file-hashes-sha-256). Any divergence indicates a modified file.

> [!NOTE]
> Hashes are not duplicated here in `README.md` to avoid drift between documents. The hash chain is regenerated whenever `src/` changes; `docs/hash-chain.md` is always current and is the binding reference for reproducibility audits.
